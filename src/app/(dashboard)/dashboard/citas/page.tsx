            ...
            {success ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  {/* Se cambia a input con datalist para poder escribir libremente y tener la lista legible */}
                  <input 
                    type="text" 
                    list="equipos-list" 
                    required 
                  ...
                  <datalist id="equipos-list">
                    <option value="Samsung Wind-Free 9000 BTU - Dormitorio" />
                    <option value="LG Dualcool 12000 BTU - Living" />
                  </datalist>
                </div>
                <div>
                  ...
                  <select value={form.tipo} onChange={e => setForm({...form, tipo: e.target.value})}
                    ...
                    <option value="Mantenimiento Preventivo" className="text-slate-800 bg-white">Mantenimiento Preventivo</option>
                    <option value="Revisión Técnica" className="text-slate-800 bg-white">Revisión Técnica</option>
                    <option value="Limpieza de Filtros" className="text-slate-800 bg-white">Limpieza de Filtros</option>
                    <option value="Reparación" className="text-slate-800 bg-white">Reparación</option>
                  </select>
                </div>
                  ...
                  <div>
                    ...
                    <select value={form.hora} onChange={e => setForm({...form, hora: e.target.value})}
                      ...
                      {["09:00","10:00","11:00","12:00","14:00","15:00","16:00","17:00"].map(h => (
                        <option key={h} value={h} className="text-slate-800 bg-white">{h}</option>
                      ))}
                    </select>
                  </div>
                </div>
                <button type="submit" className="w-full bg-orange-600 hover:bg-orange-700 text-white font-bold py-3.5 rounded-xl transition-all cursor-pointer">
                  Confirmar cita →
                </button>
              </form>
            )}
